...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, patch_size=4, in_c=204, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = x.to(torch.float)
        self.proj = self.proj.to(x.device).to(torch.float)
        x = self.proj(x)
        _, _, H, W = x.shape
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        self.norm = self.norm.to(x.device)
        x = self.norm(x)
        self.reduction = self.reduction.to(x.device)
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        self.drop1 = self.drop1.to(x.device)
        x = self.drop1(x)
        self.fc2 = self.fc2.to(x.device)
        x = self.fc2(x)
        self.drop2 = self.drop2.to(x.device)
        x = self.drop2(x)
        return x
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        relative_position_bias = relative_position_bias.to(attn.device) #在一个设备下
...
class BasicLayer(nn.Module): #实现每个stage
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=200, num_classes=9,
...
class BiLSTM(nn.Module):
    ...
    def forward(self, x):
        # print("the shape of x1",x.shape) #the shape of x1 torch.Size([16, 100])
        x = x.unsqueeze(2)
        _, (h_n, _) = self.rnn(x)
        backward_out = h_n[-1, :, :]
        forward_out = h_n[-2, :, :]
