...
import utils
import os
from model import SwinTransformer,BiLSTM


os.environ['CUDA_LAUNCH_BLOCKING'] = '0' #CUDA0
parser = argparse.ArgumentParser(description="One Shot Visual Recognition")
parser.add_argument("-d","--tar_input_dim",type = int, default = 204)
...
#这个类有什么用？？？？
class matcifar(data.Dataset): #将原始图像数据加载并转换为适合于 PyTorch 模型训练的格式
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    m = int(np.max(G))  # 9
    nlabeled = Nlabeled
    print('labeled number:', nlabeled) #5
    ...
    test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False, num_workers=0) #batch_size这么大？？
...
test_IP_data = 'D:/zyb/self_supervised1/datasets/Salinas/salinas_corrected.mat'
test_IP_label = 'D:/zyb/self_supervised1/datasets/Salinas/salinas_gt.mat'
...
class VGG16_SE(nn.Module):
    ...
    def forward(self, x):
        x = x.to(torch.float)
        self.preconv = self.preconv.to(x.device).to(torch.float)
        x=self.preconv(x)
        x=self.preconv_bn(x)
        x = self.conv1(x)
        x = self.conv2(x)
...
for episode in range(1000):
    ...
    if (episode + 1) % 1 == 0: #50
        ...
        print('episode {:>3d}:  total_loss {:6.6f},fsl_loss {:6.6f}, acc {:6.6f},'.format(
            # episode + 1, loss.item(),loss_fsl.item(),loss_rot.item(), total_hit / total_num))
            episode + 1, loss.item(), loss_fsl.item(), total_hit / total_num))
...
class Network(nn.Module):
    def __init__(self, n_bands, n_classes):
        ...
        self.spec_model =SwinTransformer(in_chans=204,
                                         patch_size=4,
                                         window_size=7,
                                         embed_dim=96,
                                         depths=(2, 2, 6, 2),
        ...
        self.fc = nn.Linear(in_features=116, out_features=64, bias=False)
    ...
    def forward(self, x=None,x1=None,x2=None,type=None):
        ...
        if(type=='ssl'):#下面那部分呗
            index = x1.shape[2]
            train_data_spec_x1 = x1[:, :, index-1, index-1]
            ...
            x2 = self.radiation_noise(x2)
            x2 = x2.to(x1.device)
            index = x2.shape[2]
            train_data_spec_x2 = x2[:, :, index-1, index-1]
...
seeds = [1220,1224,1333,1334,1233,1244,1245,1217,1330,1223]
for iDataSet in range(nDataSet):
    ...
    for episode in range(1000):
        ...
        logits = euclidean_metric(query_features, support_proto)
        query_labels = query_labels.cuda().long()
        # logits = logits.cuda().long()
        loss_fsl = crossEntropy(logits, query_labels.cuda())
        ...
        # query_labels = query_labels.to(logits.device)
        total_hit += torch.sum(torch.argmax(logits, dim=1) == query_labels).item()
        total_num += querys.shape[0]
        ...
        if (episode + 1) % 20 == 0 or episode == 0:
            ...
            #############
            embed_fea = np.empty((0, 16), dtype=np.int64)
            #############
...
#
# classification_map(hsi_pic[16:-16, 16:-16, :], best_G[16:-16, 16:-16], 24,"classificationMap/SA.png".format(Nlabeled))
